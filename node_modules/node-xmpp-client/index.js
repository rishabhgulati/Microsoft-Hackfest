/*
 * Node XMPP (Server) Client library (https://github.com/node-xmpp/node-xmpp-client)
 * Extended by Jon Francis (jon.francis@us.bosch) to implement various XEPs.
 *
 * Library Extensions:
 *
 * 06/15: Adding XMPP-PubSub (XEP-0060) functionality for server-side XMPP clients
 * | Jon -- libchange code: 001PUBSUB
 *
 * 00/00:
 * |
 */
...
var NS_PUBSUB = 'jabber:iq:pubsub'
 ...
 *   subscribe: Object - subscribe to a node | 001PUBSUB
...
Client.prototype.useFeatures = function() {
    ...
    if ((this.state === STATE_PREAUTH) && this.register) {
        ...
        if(this.options.subscribe){
          this.doSubscribe(this.options.subscribe); // 001PUBSUB
        }
    }
}
...
// 001PUBSUB
Client.prototype.trackId = function(id, callback) {
  if (!id)
    throw new this._clientError('Missing \'stanza\' id', id, callback)
  var jid
  if (typeof id === 'object') {
    if (!id.root().attrs.id)
        throw new this._clientError('Missing \'stanza\' id', id, callback)
  ...
  if (!callback)
    return this._clientError('Missing \'callback\'', id, callback)
  if (typeof callback !== 'function')
    return this._clientError('Invalid \'callback\'', id, callback)
  if (this.state !== STATE_ONLINE) {
    return this.emit('error', 'Not connected..', id, callback)
  }
 ...
 *
/*/
Client.prototype._getPubSubStanza = function(data, type, element, namespace){
  var attributes = {}
  var reqId = 'repubsub'+Math.ceil(Math.random() * 99999)
  ...
  return new ltx.Element(
    'iq',
      { to: data.to, type: type, id: reqId } //this._getId()
  ).c('pubsub', {xmlns: namespace || this.NS_PUBSUB})
   .c(element, attributes)
}

// 001PUBSUB
Client.prototype.doSubscribe = function(data, callback) {
  //if (typeof callback !== 'function')
  //  return this._clientError('Missing callback! 001PUBSUB', data)
  ...
  var self   = this
  var stanza = this._getPubSubStanza(data, 'set', 'subscribe')

  //
  this.trackId(stanza, function(stanza) {
  ...
  // */
  this.send(stanza)
  console.log("(XMPP Core) Sent \'subscribe'\ request to node \""+data.node+"\" at \""+data.to+"\"")
}
